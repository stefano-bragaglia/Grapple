...
from grapple.engine.condition import Payload, IsNode, HasLabel, IsRelation, HasType, HasProperty, HasKey, AreEqual
...
class TestCondition(TestCase):
    ...
    def test__when__validating_is_node__given__payload_with_last_node__then__true(self):
        condition = IsNode()
        assert_that(condition.is_valid(self._payload())).is_true()

    def test__when__validating_is_node__given__payload_with_last_relation__then__false(self):
        condition = IsNode()
        assert_that(condition.is_valid(self._payload()[:-1])).is_false()
    ...
    def test__when__validating_has_label__given__payload_with_last_node_without_label__then__false(self):
        ...
        assert_that(condition.is_valid(self._payload())).is_false()
    ...
    def test__when__validating_is_relation__given__payload_with_last_relation__then__true(self):
        condition = IsRelation()
        assert_that(condition.is_valid(self._payload()[:-1])).is_true()
    ...
    def test__when__validating_has_key__given__payload_with_last_relation_with_key__then_true(self):
        condition = HasKey('works_at')
        assert_that(condition.is_valid(self._payload()[:-1])).is_true()

    def test__when__validating_has_key__given__payload_with_last_relation_with_key__then_false(self):
        condition = HasKey('is_located_in')
        assert_that(condition.is_valid(self._payload()[:-1])).is_false()
